function [ Pc, A_cell, chi, val_vec, opt_vec ] = optimization_loop_klist( P, ...
    sd, EVS, A_cell, klist, parameters, fileid )
...
% -------------------------------------------------------------------------
%loop over opt_soft() for cluster numbers kt in klist.
%   [ Pc, A_cell, chi, val_vec, opt_vec ] = optimization_loop( P, sd, ...
%       EVS, A_cell, klist, parameters, fileid )
%
%   Input:
...
%                       matrices A.
%	klist		cluster numbers for which to optimize
...
%   -----------------------------------------------------------------------
    ...
    kmin = min(klist);
    kmax = max(klist);
    opt_vec = NaN(length(klist) ,2) ;
    opt_vec(:,1) = klist ;
    val_vec = NaN(length(klist), 2) ;
    val_vec(:,1) = klist ;
...
%                   vectors         
                opt_vec(find(klist == kt), 2) = (kt-val)/kt ;
                val_vec(find(klist == kt), 2) = val ;
